// ==================== بوت التداول الذكي - النسخة الكاملة مع البحث الفوري ====================
...
async function handleOpenTrade(user, exchange) {
    ...
    try {
        ...
        if (shouldExit) {
            ...
            // بعد إغلاق الصفقة، نبحث فوراً عن فرصة جديدة
            console.log(`🔍 بحث فوري عن فرصة جديدة للمستخدم ${user.telegramId}`);
...
async function findNewTradeOpportunity(user, exchange) {
    ...
    console.log(`🔍 البحث عن فرص جديدة لـ ${user.telegramId} على ${exchangeName}`);
        ...
        try {
            ...
            if (lastThreeCandles.length >= 3) {
                ...
                if (candle2Green && candle3Green && aboveSMA && currentRSI < 70) {
                    ...
                    console.log(`✅ فرصة على ${symbol} - RSI: ${currentRSI.toFixed(2)}`);
    ...
    try {
        ...
        for (const user of users) {
            try {
                ...
                // التعامل مع الصفقة المفتوحة
                if (user.currentTrade?.status === 'open') {
                    await handleOpenTrade(user, exchange);
                } 
                // البحث عن فرصة جديدة فقط إذا لم تكن هناك صفقة مفتوحة
                else {
                    const found = await findNewTradeOpportunity(user, exchange);
                    if (found) {
                        console.log(`✅ تم فتح صفقة جديدة للمستخدم ${user.telegramId}`);
                    } else {
                        console.log(`⏸️ لا توجد فرص مناسبة للمستخدم ${user.telegramId}`);
                    }
                }
...
// تشغيل التداول كل دقيقة (60 ثانية)
setInterval(executeTrading, 60 * 1000);
...
app.listen(PORT, '0.0.0.0', () => {
    ...
    console.log(`🔄 البحث عن فرص: كل دقيقة + فوري بعد كل صفقة`);
