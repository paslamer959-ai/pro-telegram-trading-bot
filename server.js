...
${user?.isRunning ? '✅ Bot Running' : '⏸️ Bot Stopped'}`, { parse_mode: 'Markdown', ...mainKeyboard });
            ...
            case '🔄 Start Bot':
                ...
                await bot.sendMessage(chatId, `✅ *Bot Started*\n\nSearching for trading opportunities using Support + RSI + Hammer strategy (3min timeframe)`, { parse_mode: 'Markdown', ...mainKeyboard });
                break;
...
async function findNewTradeOpportunity(user, exchange) {
    ...
    for (const symbol of favoriteSymbols) {
        try {
            // ✅ تم التعديل: فريم 3 دقائق، عدد الشموع 50
            const ohlcv = await exchange.fetchOHLCV(symbol, '3m', undefined, 50);
            ...
            if (nearSupport.isNear && oversold && pattern === 'hammer' && volumeConfirm) {
                ...
                console.log(`✅✅✅ Opportunity on ${symbol}: Support ${nearSupport.support} | RSI: ${rsi[rsi.length-1].toFixed(2)} | Strength: ${nearSupport.strength} | Timeframe: 3min`);
...
app.get('/health', (req, res) => res.json({ status: 'alive', mongodb: mongoose.connection.readyState === 1 ? 'connected' : 'disconnected', time: new Date().toISOString(), version: '6.0.0', strategy: 'Support + RSI<35 + Hammer + Volume + Stop under Support + 3min timeframe' }));
...
app.listen(PORT, '0.0.0.0', () => {
    ...
    console.log(`⏱️ Timeframe: 3 minutes | Candles: 50`);
    startKeepAlive();
});
