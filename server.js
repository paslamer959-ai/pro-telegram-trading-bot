...
${user?.isRunning ? '✅ Bot Running' : '⏸️ Bot Stopped'}`, { parse_mode: 'Markdown', ...mainKeyboard });
            ...
            case '🔄 Start Bot':
                ...
                await bot.sendMessage(chatId, `✅ *Bot Started*\n\nSearching for trading opportunities using Support + RSI + Hammer strategy`, { parse_mode: 'Markdown', ...mainKeyboard });
                break;
    ...
    for (const symbol of favoriteSymbols) {
        try {
            ...
            if (nearSupport.isNear && oversold && pattern === 'hammer' && volumeConfirm) {
                ...
                console.log(`✅✅✅ Opportunity on ${symbol}: Support ${nearSupport.support} | RSI: ${rsi[rsi.length-1].toFixed(2)} | Strength: ${nearSupport.strength}`);
...
app.get('/health', (req, res) => res.json({ status: 'alive', mongodb: mongoose.connection.readyState === 1 ? 'connected' : 'disconnected', time: new Date().toISOString(), version: '6.0.0', strategy: 'Support + RSI<35 + Hammer + Volume + Stop under Support' }));
...
app.listen(PORT, '0.0.0.0', () => {
    ...
    console.log(`📈 Strategy: Support + RSI<35 + Hammer + Volume + Stop under Support + Trailing Stop (No time limit)`);
    startKeepAlive();
});
