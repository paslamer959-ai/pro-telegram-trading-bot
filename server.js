...
${user?.isRunning ? '✅ Bot Running' : '⏸️ Bot Stopped'}`, { parse_mode: 'Markdown', ...mainKeyboard });
            ...
            case '❓ Help':
                ...
                const currentBalanceHelp = userStatusHelp?.totalProfit || 0;
...
• 📊 Total Trades: *${userStatusHelp?.totalTrades || 0}*
• 📈 Win Rate: *${userStatusHelp?.winRate || 0}%*

*Mean Reversion Strategy:*
• Timeframe: 15 minutes (50 candles)
• Price at least 2% below SMA20
• RSI < 30 (deep oversold)
• Target: Return to SMA20
• Stop loss: 1% below entry
...
Support: ${config.SUPPORT_CHAT}`, { parse_mode: 'Markdown', ...mainKeyboard });
            ...
            case '🔄 Start Bot':
                ...
                await bot.sendMessage(chatId, `✅ *Bot Started*\n\nSearching for mean reversion opportunities (Price 2% below SMA20 + RSI<30)`, { parse_mode: 'Markdown', ...mainKeyboard });
                break;
...
async function openNewTradeMeanReversion(user, symbol, entryPrice, stopLoss, takeProfit, exchange) {
    ...
    try {
        ...
        console.log(`🟢 Opening Mean Reversion trade on ${symbol} with ${amountToUse.toFixed(2)} USDT | Entry: ${entryPrice} | Target: ${takeProfit} | Stop: ${stopLoss}`);
...
🟢 *New Mean Reversion Trade - ${symbol}*

💰 Amount: ${amountToUse.toFixed(2)} USDT
📊 Entry Price: $${entryPrice.toFixed(4)}
📉 SMA20: $${takeProfit.toFixed(4)} (Target)
🎯 Price is ${(((takeProfit - entryPrice) / entryPrice) * 100).toFixed(2)}% below SMA20
🛑 Stop Loss: $${stopLoss.toFixed(4)} (1% below entry)
⏱️ Max Duration: 45 minutes

📈 *Strategy:* Price will revert to SMA20
...
async function handleOpenTradeMeanReversion(user, exchange) {
    ...
    try {
        ...
        // ✅ للصفقات القديمة (بدون takeProfit)
        if (!trade.takeProfit) {
            ...
            if (currentPrice <= fixedStop) {
                shouldExit = true;
                exitReason = `❌ Stop loss hit at ${fixedStop.toFixed(4)}`;
                exitPrice = fixedStop;
            }
                ...
                shouldExit = true;
                exitReason = `⏱️ Time limit reached (45 minutes)`;
            ...
            if (shouldExit) {
                ...
                console.log(`👀 ${trade.symbol} | Price: $${currentPrice} | Stop: $${fixedStop} | Trail: $${trailingStop} | High: $${highestPrice} | Time: ${minutesPassed}/45 min`);
            }
            return;
        }
        
        // ✅ للصفقات الجديدة (مع takeProfit)
        if (currentPrice >= trade.takeProfit) {
            shouldExit = true;
            exitReason = `✅ Target reached: Price returned to SMA20 (${trade.takeProfit.toFixed(4)})`;
            exitPrice = trade.takeProfit;
        }
        else if (currentPrice <= trade.stopLoss) {
            shouldExit = true;
            exitReason = `❌ Stop loss hit (1% below entry at ${trade.stopLoss.toFixed(4)})`;
            exitPrice = trade.stopLoss;
        }
        else if (minutesPassed >= 45) {
            shouldExit = true;
            exitReason = `⏱️ Time limit reached (45 minutes) - exit at current price`;
            exitPrice = currentPrice;
        }
        ...
        if (shouldExit) {
            ...
            const profitPotential = ((trade.takeProfit - currentPrice) / currentPrice) * 100;
            console.log(`👀 ${trade.symbol} | Price: $${currentPrice} | Target: $${trade.takeProfit} (${profitPotential.toFixed(2)}% to go) | Stop: $${trade.stopLoss} | Time: ${minutesPassed}/45 min`);
        }
...
async function closeTradeMeanReversion(user, exitPrice, reason, exchange) {
    ...
    try {
        ...
        await bot.sendMessage(user.telegramId, `
${emoji} *Mean Reversion Trade Closed*
...
🎯 Strategy: Price returned to SMA20
...
async function findNewTradeOpportunity(user, exchange) {
    ...
    for (const symbol of favoriteSymbols) {
        try {
            ...
            const priceExtended = currentPrice < currentSMA * 0.98;
            const oversold = currentRSI < 30;
            const volumeConfirm = checkVolumeConfirmation(volumes, volumes.length - 1);
            
            if (priceExtended && oversold && volumeConfirm) {
                const stopLoss = currentPrice * 0.99;
                const takeProfit = currentSMA;
                const potentialProfit = ((takeProfit - currentPrice) / currentPrice) * 100;
                
                console.log(`✅✅✅ Mean Reversion Opportunity on ${symbol}:`);
                console.log(`   Price: $${currentPrice} is ${(((currentSMA - currentPrice) / currentPrice) * 100).toFixed(2)}% below SMA20 ($${currentSMA})`);
                console.log(`   RSI: ${currentRSI.toFixed(2)} (<30 ✅)`);
    ...
    try {
        ...
        for (const user of users) {
            try {
                ...
                if (user.currentTrade?.status === 'open') await handleOpenTradeMeanReversion(user, exchange);
...
app.get('/health', (req, res) => res.json({ status: 'alive', mongodb: mongoose.connection.readyState === 1 ? 'connected' : 'disconnected', time: new Date().toISOString(), version: '6.0.0', strategy: 'Mean Reversion - Price 2% below SMA20 + RSI<30' }));
...
app.listen(PORT, '0.0.0.0', () => {
    ...
    console.log(`📈 Strategy: Mean Reversion - Price 2% below SMA20 + RSI<30`);
    console.log(`🎯 Target: Return to SMA20 | Stop Loss: 1% | Max Duration: 45 min`);
    startKeepAlive();
});
