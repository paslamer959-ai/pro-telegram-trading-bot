...
${user?.isRunning ? '✅ Bot Running' : '⏸️ Bot Stopped'}`, { parse_mode: 'Markdown', ...mainKeyboard });
            ...
            case '🔄 Start Bot':
                ...
                await bot.sendMessage(chatId, `✅ *Bot Started*\n\nSearching for trading opportunities using Support + RSI + Volume strategy (1min timeframe)`, { parse_mode: 'Markdown', ...mainKeyboard });
                break;
...
async function findNewTradeOpportunity(user, exchange) {
    ...
    for (const symbol of favoriteSymbols) {
        try {
            ...
            // ✅ تم استبعاد شرط Hammer
            if (nearSupport.isNear && oversold && volumeConfirm) {
                const stopLoss = nearSupport.support * 0.995;
                console.log(`✅✅✅ Opportunity on ${symbol}: Support ${nearSupport.support} | RSI: ${rsi[rsi.length-1].toFixed(2)} | Strength: ${nearSupport.strength} | Volume: ${volumeConfirm ? 'High' : 'Low'} | Timeframe: 1min`);
...
app.get('/health', (req, res) => res.json({ status: 'alive', mongodb: mongoose.connection.readyState === 1 ? 'connected' : 'disconnected', time: new Date().toISOString(), version: '6.0.0', strategy: 'Support + RSI<35 + Volume + Stop under Support + 1min timeframe' }));
...
app.listen(PORT, '0.0.0.0', () => {
    ...
    console.log(`📈 Strategy: Support + RSI<35 + Volume + Stop under Support + Trailing Stop`);
