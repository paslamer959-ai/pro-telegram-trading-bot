...
function checkVolumeConfirmation(volumes, index, period = 4) {
    ...
    return currentVol > avgVol * 1.2; // حجم أكبر 20% فقط
}
...
async function openNewTradeMeanReversion(user, symbol, entryPrice, stopLoss, takeProfit, exchange) {
    ...
    try {
        ...
        await User.findOneAndUpdate({ telegramId: user.telegramId }, {
            $set: {
                currentTrade: { symbol, exchange: user.selectedExchange, entryPrice, takeProfit, stopLoss, entryTime: new Date(), orderId: order.id, status: 'open' },
...
async function findNewTradeOpportunity(user, exchange) {
    ...
    for (const symbol of favoriteSymbols) {
        try {
            ...
            // ✅ شرط السعر الممتد: السعر أقل من SMA20 بـ 2% على الأقل
            ...
            if (priceExtended && oversold && volumeConfirm) {
                const stopLoss = currentPrice * 0.99; // 1% تحت الدخول
